...
```js
const candidateSwitchCheckbox = document.getElementById("candidate-switch")
let selectedCell
...
document.querySelectorAll(".number-control").forEach(button => {
  button.addEventListener("click", () => {
    ...
    if (candidateSwitchCheckbox.checked) {
      let candidatesArray = selectedCell
      ...
      selectedCell.querySelector(".candidates").textContent =
        candidatesArray.join("")
    } else {
